...
private:
  using Topic = shm_msgs::msg::PointCloudOuster;

public:
  explicit Listener(const rclcpp::NodeOptions &options)
      : Node("shm_pcl_ouster_listener", options) {
    ...
    auto callback = [this](const Topic::SharedPtr msg) -> void {
      ...
      RCLCPP_INFO(get_logger(), "get-pcl_ouster-transport-time: %.3f", transport_time_ms);
      if(m_last_cloud_ts.nanoseconds() > 0.0)
      {
        RCLCPP_INFO(get_logger(), "get-pcl_ouster-timestamp_offset-time: %.3f", timestamp_offset_ms);
      }
